...
`sdm` is **reconcile-and-report**, Terraform/`make` style: every `sync` re-derives all output
...
See [`sdm-design.md`](sdm-design.md) for the full design rationale.
...
Hand-maintaining both, in sync, across a monorepo is error-prone. `sdm` makes one YAML file
...
```sh
go build -o sdm .   # or: go install .
```

The tool operates on `services.yaml` in the **current directory** (or `-C <dir>`); there is no
environment variable to configure. Check the version with `sdm version`.
...
sdm host add resolver       --ip 192.0.2.1   --dir resolver
sdm host add appbox --ip 192.0.2.2 --dir appbox

# 2. Declare the domains and the Caddy tls snippet each one imports
sdm domain add example.com --tls-import tls_example_com
sdm domain add example.net --tls-import tls_example_net

# 3. Add a service (mutates YAML, then syncs)
sdm add docs --fqdn docs.example.com --host appbox --backend paperless:8000
...
# GENERATED by sdm — do not edit. Source: services.yaml
...
sdm [-C <dir>] domain remove <name>
  ...
  -C <dir>   run as if sdm were started in <dir> (default: current directory)
```
...
`sdm` **owns** this file and rewrites it wholesale on mutation. Ordering and human comments
...
`sdm-manifest.yaml` (repo root, **committed to git**) records which files each service
...
`sdm` only writes into the `sites/` directory; it never edits a machine's main `Caddyfile`.
...
Caddy serves wildcard certs from an external acme.sh pipeline — `sdm` never touches certs or
ACME.
