# sdm (Split-horizon DNS Manager) — Design Document
...
- The `tls_import` for a service is chosen by matching its `fqdn`'s registrable domain suffix
  against the `domains` map.
...
- `import tls_<domain>` is selected from `domains[<suffix>].tls_import`.
...
### 4.3 Caddyfile integration (one-time, manual, documented — not done by the tool)
Each machine's main `Caddyfile` should contain `import sites/*.caddy` plus the static
`(tls_<domain>)` snippet definitions. The tool only writes files into `sites/`; it never edits
the main Caddyfile. Document this as a setup prerequisite.

## 5. Manifest

- **One file**, at the repo root (e.g. `sdm-manifest.yaml`), **committed to git**.
